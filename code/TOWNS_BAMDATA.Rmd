...
wd = getwd()
setwd("../input/TOWNS/Centerline/")
Willamette_cl <- shapefile("Olen_cl_UTM.shp")
setwd(wd)
Willamette_PT_coords <- read.csv("../input/TOWNS/PT_coords/Olen_PT_coords.csv")
...
Willamette_orthos <- ortho_lines(Willamette_DEM, data.frame(Willamette_cl@lines[[1]]@Lines[[1]]@coords), 100)
...
for(i in 1:length(Willamette_elev_prof)){
  Willamette_scale_x[i] = 200/length(Willamette_elev_prof[[i]])
}

plot_elev_prof(Willamette_elev_prof, Willamette_WSE_daily, "Willamette")
#locator(n = 4)
Willamette_upper_l <- c( 8.2,  8.1,  3.2,  3.3, 11.5,  4.6, 11.2, 10.3, 11.4,  7.1,  6.5, 11.3,  9.8,  1.4, 12.3,  8.7,  8.2,  8.9,  6.3, 11.5)
Willamette_lower_l <- c(10.5, 10.2,  5.2,  5.3, 13.2,  7.1, 13.3, 12.3, 13.3,  9.8, 10.3, 13.2, 11.3, 13.1, 14.3, 10.6, 10.4, 11.3,  7.9, 13.3)
Willamette_lower_r <- c(15.7, 18.5, 18.4, 25.6, 21.6, 12.7, 17.7, 18.7, 18.6, 19.5, 16.6, 20.5, 20.7, 15.7, 20.7, 15.8, 16.7, 16.4, 21.8, 20.0)
Willamette_upper_r <- c(19.4, 21.1, 21.2, 27.5, 23.7, 15.8, 20.8, 23.4, 22.0, 21.6, 20.4, 22.8, 22.5, 18.1, 24.6, 18.0, 20.5, 20, 23.7, 23.1)
...
xvec <- rev(calc_xvec(Willamette_PT_coords_sp, Willamette_cl))
